...
def process_system_analysis(auth_url, request_url, api_token, xml_directory, output_base_dir, tag_name,
    ...
    try:
        ...
        def show_excel_test_mode(logs2, file_prefix):
            # Combine logs into a single DataFrame
            combined_logs = []
            for factsheet_type, log_entries in logs2.items():
                for entry in log_entries:
                    combined_entry = entry.copy()
                    combined_entry['FactSheet Type'] = factsheet_type
                    combined_logs.append(combined_entry)

            # Create DataFrame from the logs
            combined_df = pd.DataFrame(combined_logs)
            ...
            output_dir = os.path.join('path_to_output_directory', timestamp + '_test')
            os.makedirs(output_dir, exist_ok=True)
            file_path222 = os.path.join(output_dir, f"{file_prefix}.xlsx")

            try:
                with pd.ExcelWriter(file_path222, engine='xlsxwriter') as writer:
                    combined_df.to_excel(writer, sheet_name='Logs', index=False)
                print(f"Test mode logs successfully saved to {file_path222}")
            except Exception as e:
                print(f"Failed to save test mode logs: {e}")
        ...
        if recovery:
            ...
            # Step 2: Check for 'created_factsheets.xlsx' in the same folder, but don't raise an error if it's missing
            ...
            if os.path.exists(created_factsheets_path):
                ...
                print("'created_factsheets.xlsx' not found. Skipping archive task for created factsheets.")
        ...
        if test_mode and delete_operation:
            ...
            if fact_sheets_to_archive:
                ...
                for fact_sheet in fact_sheets_to_archive:
                    ...
                    # Properly log the fact sheets in the 'logs' dictionary for this operation
            ...
            # After all fact sheets are processed, save logs to Excel
            if any_factsheets_deleted:
                file_path3 = show_excel_test_mode(logs, "deleted_factsheets")
                return file_path3
        ...
        if test_mode and create_factsheet:
            ...
            for run in range(3):  # Run the creation operation three times
                ...
                logs['Application'].extend(
                    [{"Action": "Create", "FactSheet Type": "Application", "Name": app} for app in
                ...
                existing_it_component_names = {normalize_name(component['node']['name']): component['node'] for
                                               component in
            ...
            file_path2 = show_excel_test_mode(logs, "created_factsheets")
            return file_path2
        ...
        if test_mode and delta_operation:
            ...
            for run in range(3):
                ...
                # Save logs to Excel
            file_path1 = show_excel_test_mode(logs, "Operation")
            return file_path1
            ...
            try:
                for _ in range(3):  # Loop to run the process three times
                    ...
                    def process_modules_data(modules_data1, existing_business_capabilities1):
                        ...
                        print(f"Business Capabilitie factsheet {unique_business_capability_names} created")
