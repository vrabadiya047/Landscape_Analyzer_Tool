...
class RecoveryDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # Create input fields similar to ImportDialog
        self.host_name_input, host_name_layout = self.create_input_field("Sub-domain Name:")
        layout.addLayout(host_name_layout)
...
class LandscapeAnalyzer(QMainWindow):
    ...
    def display_reports(self):
        ...
        # Initialize the table with 6 columns: Timestamp, Duration, Created Factsheet, Updated Factsheet, Deleted Factsheet, Action
        self.reports_table = QTableWidget(len(directories),
                                          6)  # 6 columns: Timestamp, Duration, Created, Updated, Deleted, Action
        self.reports_table.setHorizontalHeaderLabels(
            ["Date & Time", "Duration", "Action", "Created Factsheet", "Updated Factsheet", "Deleted Factsheet"])
        ...
        if directories:
            for row, directory in enumerate(directories):
                ...
                # Read the duration from the file saved in the same timestamped folder
                duration_file = os.path.join(base_dir, directory, "duration.txt")
                if os.path.exists(duration_file):
                    with open(duration_file, 'r') as f:
                        duration = f.read().strip()
                else:
                    duration = "N/A"  # If the duration file is not found, display "N/A"

                # Add the duration to the Duration column
                duration_item = QTableWidgetItem(duration)
                duration_item.setFlags(duration_item.flags() & ~Qt.ItemIsEditable)
                duration_item.setForeground(QBrush(QColor("#FFFFFF")))
                duration_item.setTextAlignment(Qt.AlignCenter)
                self.reports_table.setItem(row, 1, duration_item)
                ...
                self.reports_table.setItem(row, 3, created_item)
                ...
                self.reports_table.setItem(row, 4, updated_item)
                ...
                self.reports_table.setItem(row, 5, deleted_item)
                ...
                self.reports_table.setItem(row, 2, action_item)
