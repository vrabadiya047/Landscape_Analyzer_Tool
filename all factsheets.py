...
def process_system_analysis(auth_url, request_url, api_token, xml_directory, output_base_dir, tag_name,
    ...
    def save_created_updated_factsheets_to_excel(created_factsheets, updated_factsheets, deleted_factsheets):
        ...
        created_df = pd.DataFrame(created_factsheets).drop_duplicates(subset=[name])
        updated_df = pd.DataFrame(updated_factsheets).drop_duplicates(subset=[name])
        deleted_df = pd.DataFrame(deleted_factsheets).drop_duplicates(subset=[name])
    ...
    if delta_operation:
        for _ in range(3):  # Loop to run the process three times
            ...
            for norm_app_name, app_name in normalized_xml_applications.items():
                if norm_app_name in existing_app_names:
                    ...
                    if current_description != new_description:
                        ...
                        print(f"Application factsheet updated: {app_name} with new description: {new_description}")
            ...
            for component in components_data:
                ...
                if existing_component:
                    ...
                    if current_description != new_description:
                        if update_it_component_if_changed(component_id, component_display_name, new_description):
            ...
            for existing_component in leanix_it_components:
                ...
                if component_display_name not in new_it_components:
                    if any(tag['name'] == tag_name for tag in existing_component['tags']) and existing_component[
            ...
            it_components_display_names = {f"{comp['name']} {comp['release']}": comp for comp in leanix_it_components}

            # Retrieve all applications again after creation
            applications_response = requests.post(url=request_url, headers=headers, json={"query": application_query})
            ...
            response6 = requests.post(url=request_url, headers=headers, json={"query": technical_stack_query})
            response6.raise_for_status()
            leanix_technical_stacks = [node['node'] for node in response6.json()['data']['allFactSheets']['edges']]
            ...
            response10 = requests.post(url=request_url, headers=headers, json={"query": existing_relationships_query})
            ...
            for client in clients_data:
                ...
                if norm_name not in existing_applications_dict:
                    logs['Application'].append({
                        "Action": "Created",
                        "FactSheet Type": "Application",
                ...
                if norm_name in existing_applications_dict:
                    ...
                    if new_description != current_description:
                        ...
                        print(f"Application factsheet {it_component_name} with description {new_description} updated")

                    application_level2.append({"name": it_component_name, "id": app_id, "description": new_description})
                else:
            ...
            for system in modules_data:
                factsheet_name = system['belongs_to']  # Assume the 'name' key is the identifier for the factsheet
            ...
            response = requests.post(url=request_url, headers=headers, json={"query": existing_application_query})
            response.raise_for_status()
            ...
            def get_existing_business_capabilities():
                ...
                response = requests.post(url=request_url, headers=headers, json={"query": query})
                response.raise_for_status()
                return {data['node']['name']: {'id': data['node']['id'], 'tags': data['node']['tags']} for data in
                        response.json()['data']['allFactSheets']['edges']}
            ...
            la_system_tagged_bc = [bc for bc in existing_business_capabilities if
                                   any(tag['name'] == tag_name for tag in existing_business_capabilities[bc]['tags'])]
            ...
            def create_relationship(app_id2, app_name, biz_cap_id, biz_cap_name):
                ...
                response.raise_for_status()
                print(f"Relationship created between Application '{app_name}' and Business Capability '{biz_cap_name}'")
            ...
            # Create Data Object FactSheet if it does not exist
            ...
            # Convert the list of new interfaces to a DataFrame
            ...
            def fetch_all_interfaces():
                ...
                response.raise_for_status()
                result = response.json()
                return {edge['node']['name']: {'id': edge['node']['id'], 'tags': edge['node']['tags']} for edge in
                        result['data']['allFactSheets']['edges']}
            ...
            interfaces_to_delete = [name for name, details in existing_interfaces.items()
                                    if tag_name in [tag['name'] for tag in
                                                    details['tags']] and name.lower() not in new_interface_names]
            ...
            for _, relationship in matched_df5.iterrows():
                ...
                application_id = applications.get(application_name)
                if data_object_id and application_id and (
                        data_object_id, application_id) not in existing_relationships_data_object_to_application:
            ...
            # Assuming Rfc_data, clients_data, and Ale_data are already defined
            additional_relationships_data = match_rfc_to_ale_for_additional_relationships(Rfc_data, clients_data,
                                                                                          Ale_data)
        ...
        operation_logs_path = save_factsheet_logs_to_excel(logs, "Operation", test_mode=False)
    ...
    if create_factsheet:
        for _ in range(3):  # Loop to run the process three times
            ...
            response3 = requests.post(url=request_url, headers=headers, json={"query": component_query})
            response3.raise_for_status()
            leanix_it_components = [node['node']['name'] for node in response3.json()['data']['allFactSheets']['edges']]
            ...
            def create_factsheet(factsheet_name):
                ...
                return response17.json()

            # Iterate over module data to create or update factsheets
            for system in modules_data:
                factsheet_name = system['belongs_to']  # Assume the 'belongs_to' key is the identifier for the factsheet
                sub_module = system['acronym'] + "(" + system['sid'] + "-" + system['Client_id'] + ")"

                if sub_module not in existing_names:
                    result3 = create_factsheet(sub_module)
                    print(f" Application factsheet {sub_module} Created")
